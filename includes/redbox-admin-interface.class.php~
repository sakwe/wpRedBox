...
class RedBoxAdmin{
	...
	public function display_redbox_dashboard_widget(){
		...
		$widget.= '</textarea>';
		$widget.= '<div id="redbox_widget_status" style=""></div>';
		$widget.= '<input style="display:inline-block;text-align:right;" type="button" class="redbox_button redbox_button-info" onclick="redbox_ajax_do(\'redbox_submit_from_adminbar\',document.getElementById(\'redbox_textarea\').value,true,false)" value="'.$txtBtn.'" />';
		$widget.= '';
		$widget.= '</form>';
		$widget.= '</div>';
		echo $widget;
	...
	public function redbox_import_options() {
		...
		echo '<li><span>'.REDBOX_FB_POST_SIGN.' : </span>
			<input type="text" name="redbox_fb_post_sign" class="redbox_config_imputbox" value="'.$options['redbox_fb_post_sign'].'" />
			</li>';
		echo '<li><span>'.REDBOX_AUTOTAGS_LIST.' : </span>
			<textarea name="facebook_tags_field" class="redbox_config_textarea">'.$options['facebook_tags_for_posts'].'</textarea>
			</li>';
		echo '<li><span>'.REDBOX_CROWDFOUNDING.' : </span>
			<textarea name="redbox_crowdfundings" class="redbox_config_textarea">'.$options['redbox_crowdfundings'].'</textarea>
			</li>';
		echo '<li><span>'.REDBOX_TO_CLEAN_IN_URLS.' : </span>
			<textarea name="redbox_to_clean_in_urls" class="redbox_config_textarea">'.$options['redbox_to_clean_in_urls'].'</textarea>
			</li>';
		echo '<li><span>'.REDBOX_SUB_REPLACE_SOURCE.' : </span>
			<textarea name="redbox_sub_replace_source" class="redbox_config_textarea">'.$options['redbox_sub_replace_source'].'</textarea>
			</li>';
		echo '<li><span>'.REDBOX_TO_CLEAN_IN_TITLES.' : </span>
			<textarea name="redbox_to_clean_in_titles" class="redbox_config_textarea">'.$options['redbox_to_clean_in_titles'].'</textarea>
			</li>';
		echo '<li><span>'.REDBOX_TO_CLEAN_IN_TEXTS.' : </span>
			<textarea name="redbox_to_clean_in_texts" class="redbox_config_textarea">'.$options['redbox_to_clean_in_texts'].'</textarea>
			</li>';
		echo '</ul></div>';
